...
static word12 banner_card[] = {
 00005, 00000, 00000, 07700, 07704, 07704, 07704, 07700, 00000, 00000,
 00077, 00477, 00477, 00477, 00077, 00000, 00000, 07700, 07704, 07704,
 07704, 07700, 00000, 00000, 00077, 00477, 00477, 00477, 00077, 00000,
 00000, 07700, 07704, 07704, 07704, 07700, 00000, 00000, 00077, 00477,
 00477, 00477, 00077, 00000, 00000, 07700, 07704, 07704, 07704, 07700,
 00000, 00000, 00077, 00477, 00477, 00477, 00077, 00000, 00000, 07700,
 07704, 07704, 07704, 07700, 00000, 00000, 00077, 00477, 00477, 00477,
 00077, 00000, 00000, 07700, 07704, 07704, 07704, 07700, 00000, 00005,
};
...
static word12 end_of_deck_card[] = {
 00005, 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00000,
 00000, 00000, 00000, 00000, 00000, 00000, 00000, 02000, 02400, 02400,
 02400, 03700, 00000, 00000, 03721, 02112, 02104, 02104, 03737, 00000,
 00000, 00021, 00021, 00021, 00021, 00037, 00000, 00000, 01621, 02125,
 02125, 02125, 03737, 00000, 00000, 03716, 00221, 00421, 01021, 03737,
 00000, 00000, 02100, 02500, 02500, 02500, 03700, 00000, 00000, 00000,
 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00000,
 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00000, 00005,
};
...
static int
mcc_to_ascii(word12 punch_code)
{
...
// Scans data cards to determine if
// they contain all valid MCC punch codes
static bool
check_for_valid_mcc_cards(void)
{
...
static void
convert_mcc_to_ascii(word12 *buffer, char *ascii_string)
{
 ...
 *
 * This is the parsing of the "lace" cards and
 * extracting the ASCII characters have been
 * punched into the cards (as glyphs) so the
 * operator knows how to deliver the deck.
 */
...
static void
print_card(FILE *out_file, card_image_t *card, bool flip_card)
{
 if (output_cards)
   {
    fprintf(
          ...
          for (int col = CARD_COL_COUNT - 1; col >= 0; col--)
            {
             fprintf(
              out_file,
              card->column[col] & row_bit_masks[row] ? "*" : " ");
            }
         }
       else
         {
...
static void
log_char_matrix_pattern(uint8 *char_matrix)
{
 fprintf(stderr, "\nChar Matrix\n");
...
static char
search_glyph_patterns(uint8 *matrix)
{
 for (int pattern = 0; pattern < NUM_GLYPH_CHAR_PATTERNS; pattern++)
   {
    if (memcmp(matrix, &glyph_char_patterns[pattern], CHAR_MATRIX_BYTES)
        == 0)
      {
...
static char
get_lace_char(const word12 *buffer, uint char_pos)
{
 if (char_pos >= GLYPHS_PER_CARD)
   {
    fprintf(stderr,
     "\n*** Error: Attempt to read punch block character out of range "
     "(%u)\n", char_pos);
    _Exit(4);
   }

 bool top         =   char_pos < 11; // Top or bottom line of characters
 uint char_offset = ( char_pos < 11 )
                    ? char_pos
                    : char_pos - 11; // Character number in the line
 word12 col_buffer[5];      // The extracted 5 columns for the character

 // Extract the five 12-bit words from the main buffer that make up the
 // character Note that in this process we reverse the character image
 // so it reads normally (characters are punched in reverse)
 for (uint col_offset = 0; col_offset < 5; col_offset++)
   {
    col_buffer[4 - col_offset]
     = buffer[glyph_starting_column[char_offset] + col_offset];
   }
 ...
 for (uint col_offset = 0; col_offset < CHAR_MATRIX_BYTES; col_offset++)
   {
    char_matrix[col_offset]
     = ( col_buffer[col_offset] >> ( top ? 6 : 0 )) & 0x1F;
   }
...
static void
scan_card_for_glyphs(card_image_t *card)
{
...
allocate_card(void)
{
 ...
 if (card == NULL)
   {
    fprintf(stderr, "\n*** Error: Failed to allocate memory for card image!\r\n");
...
read_card(FILE *in_file)
{
 ...
 if (bytes_read == 0)
   {
    if (feof(in_file))
      ...
      {
       fprintf(stderr, "\n*** Error: fread returned zero but failed to set "
        "the error or eof flags!\n");
       _Exit(2);
      }
   }
 ...
 if (bytes_read != BYTES_PER_CARD)
   {
    fprintf(stderr,
     "\n*** Error: failed to read a full card (only read %d of %d bytes)\n",
     bytes_read, BYTES_PER_CARD);
    _Exit(3);
   }

 // We have a full card so allocate a card and convert the byte buffer into
 // the card image
 card_image_t *card = allocate_card();

 for (int current_nibble = 0; current_nibble < NIBBLES_PER_CARD;
      current_nibble++)
   {
    ...
    word12  nibble        = ( current_nibble & 0x1 )
                            ? byte_buffer[byte_offset]
                          : ( byte_buffer[byte_offset] >> 4 );
...
static void
save_card_in_cache(CARD_CACHE *card_cache, card_image_t *card)
{
...
static void
print_event(enum parse_event event)
{
 switch (event)
   {
   case NoEvent:
...
static void
print_state(enum parse_state state)
{
 switch (state)
   {
   case Idle:
...
static void
transition_state(enum parse_event event, enum parse_state new_state)
{
 if (output_debug)
   {
...
static enum parse_event
do_state_idle(enum parse_event event)
{
...
static enum parse_event
do_state_starting_job(enum parse_event event, card_image_t *card)
{
...
static enum parse_event
do_state_scan_card_for_glyphs(enum parse_event event, card_image_t *card)
{
...
static enum parse_event
do_state_end_of_header(enum parse_event event, card_image_t *card)
{
...
static enum parse_event
do_state_cache_card(enum parse_event event, card_image_t *card)
{
...
static enum parse_event
do_state_end_of_deck(enum parse_event event, card_image_t *card)
{
...
static enum parse_event
do_state_end_of_job(enum parse_event event, card_image_t *card)
{
...
static void
unexpected_event(enum parse_event event)
{
 if (output_debug)
   {
 ...
 while (event != NoEvent)
   {
    ...
    switch (current_event)
      ...
      default:
       fprintf(stderr, "\n*** Error: Punch received unknown event!\n");
       break;
      }
   }
}
...
print_help(char *program)
{
 ...
 printf("1.8.1  or  later.  For  complete  details,  see  the \"LICENSE.md\" included,  or\n");
 printf("visit https://gitlab.com/dps8m/dps8m/-/blob/master/LICENSE.md.\n");
}
  ...
  while (!done)
    {
      ...
      poptContext opt_con;
      opt_con
       = poptGetContext(NULL, argc, (const char **)argv,
                        long_options, 0);
      while (( c = poptGetNextOpt(opt_con)) > 0)
        {
#else  /* ifdef USE_POPT */
      int option_index = 0;
      c = getopt_long(argc, argv, "7acdfghmnrv",
         ...
         switch (c)
           ...
           case '7':
            fprintf(stderr, "\n*** Sorry: 7punch format is not yet supported!\n");
            _Exit(1);
           ...
           default:
            fprintf(stderr,
             "\n*** Internal Error: did not recognize option when "
             "parsing options, got %d\n", c);
            _Exit(1);
           }
#ifdef USE_POPT
        }
      ...
      bool override_in_effect
       = output_cards || output_glyphs || output_mcc || output_raw;

      if (!output_auto && !override_in_effect)
        {
         fprintf(stderr,
          "\n*** Error: auto mode was disabled with no override "
          "mode selected!\n");
         _Exit(1);
        }
    }
   ...
   while (current_entry != NULL)
     {
      ...
      for (int current_nibble = 0; current_nibble < NIBBLES_PER_CARD;
           current_nibble += 2)
        {
         ...
         uint   nibble
                        = ( current_entry->card->column[col] \
                                 >> ( nibble_offset * 4 )) & 0x00F;
         byte          |= nibble << 4;

         col            = ( current_nibble + 1 ) / 3;
         nibble_offset  = 2 - (( current_nibble + 1 ) % 3 );

         nibble         = ( current_entry->card->column[col] \
                                 >> ( nibble_offset * 4 )) & 0x00F;
         byte          |= nibble;
...
/* ######################################################################### */
  ...
  main(int argc, char *argv[])
  {
   ...
   if (output_auto)
     {
      if (check_for_valid_mcc_cards())
      ...
      else
        {
         output_raw = true;
         fprintf(stderr,
          ">> Auto-detection did not recognize format so "
          "output mode set to raw <<\n");
        }
     }
   ...
   if (output_cards)
     {
      fprintf(stderr,
       "\n*****\nWriting ASCII Art Card Images (banner cards%s "
       "flipped)\n*****\n", output_flip ? "" : " not");
      dump_cards(stdout);
     }
