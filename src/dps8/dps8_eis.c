...
//
//
//  EISWriteIdx (cpuPtr, p, n, data, flush); -- write to cache at p->addr [n];
//  EISRead (cpuPtr, p) -- read to cache from p->addr
//  EISReadIdx (cpuPtr, p, n)  -- read to cache from p->addr[n];
//  EISReadN (p, n, dst) -- read N words to dst;

//  EISget469 (cpuPtr, k, i)
//  EISput469 (cpuPtr, k, i, c)

//  EISget49 (cpuPtr, k, *pos, tn) get p->addr[*pos++]
...
static word36 getCrAR (cpu_state_t *cpuPtr, word4 reg)
  {
    if (reg == 0)
      return 0;
...
static word18 getMFReg18 (cpu_state_t *cpuPtr, uint n, bool allowDU, bool allowNIC, fault_ipr_subtype_ *mod_fault)
  {
    switch (n)
      {
        case 0: // n
          if (! allowNIC)
            {
              ...
              *mod_fault |= FR_ILL_MOD;
              //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg18 n");
            }
          return 0;
...
sim_printf ("getMFReg18 %012"PRIo64"\n", IWB_IRODD);
              ...
              *mod_fault |= FR_ILL_MOD;
              //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg18 du");
            }
          return 0;
          ...
          if (! allowNIC)
            {
              //sim_printf ("getMFReg18 n\n");
              *mod_fault |= FR_ILL_MOD;
              //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg18 ic");
            }
          return cpu.PPR.IC;
        ...
        case 7: // dl
          *mod_fault |= FR_ILL_MOD;
          //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg18 dl");
          return 0;
...
static word36 getMFReg36 (cpu_state_t *cpuPtr, uint n, bool allowDU, bool allowNIC, fault_ipr_subtype_ *mod_fault)
  {
    switch (n)
      {
        case 0: // n
         if (! allowNIC)
           {
             ...
             *mod_fault |= FR_ILL_MOD;
             //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg36 n");
           }
          return 0;
        case 1: // au
          return GETHI (cpu.rA);
        ...
        case 3: // du
          ...
          if (! allowDU)
           *mod_fault |= FR_ILL_MOD;
           //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg36 du");
          return 0;
          ...
          if (! allowNIC)
            {
              //sim_printf ("getMFReg36 n\n");
              *mod_fault |= FR_ILL_MOD;
              //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg36 ic");
            }
          return cpu.PPR.IC;
        ...
        case 7: // dl
             *mod_fault |= FR_ILL_MOD;
             //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "getMFReg36 dl");
             return 0;
...
static void EISWriteCache (cpu_state_t *cpuPtr, EISaddr * p)
  {
...
sim_debug (DBG_TRACEEXT, & cpu_dev, "EIS %ld Write8 TRR %o TSR %05o\n", eisaddr_idx, cpu.TPR.TRR, cpu.TPR.TSR); }
            ...
            for (uint i = 0; i < 8; i ++)
              if (p->wordDirty[i])
                {
                  Write1 (cpuPtr, p->cachedAddr+i, p -> cachedParagraph[i], true);
                  p->wordDirty[i] = false;
                }
#else
...
sim_debug (DBG_TRACEEXT, & cpu_dev, "EIS %ld Write8 NO PR TRR %o TSR %05o\n", eisaddr_idx, cpu.TPR.TRR, cpu.TPR.TSR); }
            ...
            for (uint i = 0; i < 8; i ++)
              if (p->wordDirty[i])
                {
                  Write1 (cpuPtr, p->cachedAddr+i, p -> cachedParagraph[i], false);
                  p->wordDirty[i] = false;
                }
#else
...
static void EISReadCache (cpu_state_t *cpuPtr, EISaddr * p, word18 address)
  {
...
sim_debug (DBG_TRACEEXT, & cpu_dev, "EIS %ld Read8 TRR %o TSR %05o\n", eisaddr_idx, cpu.TPR.TRR, cpu.TPR.TSR); }
        Read8 (cpuPtr, paragraphAddress, p -> cachedParagraph, true);
...
sim_debug (DBG_TRACEEXT, & cpu_dev, "EIS %ld Read8 NO PR TRR %o TSR %05o\n", eisaddr_idx, cpu.TPR.TRR, cpu.TPR.TSR); }
        Read8 (cpuPtr, paragraphAddress, p -> cachedParagraph, false);
        if_sim_debug (DBG_TRACEEXT, & cpu_dev)
          {
            for (uint i = 0; i < 8; i ++)
              sim_debug (DBG_TRACEEXT, & cpu_dev,
...
static void EISWriteIdx (cpu_state_t *cpuPtr, EISaddr *p, uint n, word36 data, bool flush)
{
#ifdef EIS_PTR
    long eisaddr_idx = EISADDR_IDX (p);
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
    ...
    if ((! p -> cacheValid) || p -> cachedAddr != paragraphAddress)
      {
        EISReadCache (cpuPtr, p, paragraphAddress);
      }
    p -> cacheDirty = true;
#ifdef CWO
...
static word36 EISReadIdx (cpu_state_t *cpuPtr, EISaddr * p, uint n)
  {
#ifdef EIS_PTR
    long eisaddr_idx = EISADDR_IDX (p);
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
...
static word36 EISRead (cpu_state_t *cpuPtr, EISaddr * p)
  {
#ifdef EIS_PTR
    long eisaddr_idx = EISADDR_IDX (p);
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
...
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
    ...
    for (uint n = 0; n < N; n ++)
      {
        * dst ++ = EISReadIdx (cpuPtr, p, n);
      }
  }
#endif

static void EISReadPage (cpu_state_t *cpuPtr, EISaddr * p, uint n, word36 * data)
  {
#ifdef EIS_PTR
    long eisaddr_idx = EISADDR_IDX (p);
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
    ...
    if (p -> mat == viaPR)
      {
        ...
        cpu.TPR.TSR = p -> SNR;
        cpu.cu.XSF = 0;
        ReadPage (cpuPtr, addressN, data, true);
    ...
    else
      {
        ...
        ReadPage (cpuPtr, addressN, data, false);
        if_sim_debug (DBG_TRACEEXT, & cpu_dev)
          {
            for (uint i = 0; i < PGSZ; i ++)
              sim_debug (DBG_TRACEEXT, & cpu_dev,
...
static void EISWritePage (cpu_state_t *cpuPtr, EISaddr * p, uint n, word36 * data)
  {
#ifdef EIS_PTR
    long eisaddr_idx = EISADDR_IDX (p);
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
    ...
    if (p -> mat == viaPR)
      {
        ...
        cpu.TPR.TSR = p -> SNR;
        cpu.cu.XSF = 0;
        WritePage (cpuPtr, addressN, data, true);
    ...
    else
      {
        ...
        WritePage (cpuPtr, addressN, data, false);
        if_sim_debug (DBG_TRACEEXT, & cpu_dev)
          {
            for (uint i = 0; i < PGSZ; i ++)
              sim_debug (DBG_TRACEEXT, & cpu_dev,
...
static word9 EISget469 (cpu_state_t *cpuPtr, int k, uint i)
  {
...
#endif
    word36 data = EISRead (cpuPtr, & e -> addr [k - 1]);    // read it from memory
...
static void EISput469 (cpu_state_t *cpuPtr, int k, uint i, word9 c469)
  {
    EISstruct * e = & cpu.currentEISinstruction;
    ...
    EISWriteIdx (cpuPtr, & e -> addr [k - 1], 0, w, true);
  }
...
static word9 EISget49 (cpu_state_t *cpuPtr, EISaddr * p, int * pos, int tn)
  {
...
#endif
        p -> data = EISRead (cpuPtr, p);    // read it from memory
      }
    else
      {
        p -> data = EISRead (cpuPtr, p);   // read data word from memory
      }
...
static bool EISgetBitRWN (cpu_state_t *cpuPtr, EISaddr * p, bool flush)
  {
#ifdef EIS_PTR
    long eisaddr_idx = EISADDR_IDX (p);
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
    ...
    if (p -> mode == eRWreadBit)
    ...
    else if (p -> mode == eRWwriteBit)
      {
        ...
        EISWriteIdx (cpuPtr, p, 0, p -> data, flush); // write data word to memory
      }
...
static void setupOperandDescriptorCache (cpu_state_t *cpuPtr, int k)
  {
    EISstruct * e = & cpu.currentEISinstruction;
...
static void setupOperandDescriptor (cpu_state_t *cpuPtr, int k, fault_ipr_subtype_ *mod_fault)
  {
    EISstruct * e = & cpu.currentEISinstruction;
    switch (k)
      {
...
sim_printf ("setupOperandDescriptor %012"PRIo64"\n", IWB_IRODD);
...
#endif
            doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "setupOperandDescriptor 18-28,30, 31 MBZ");
          }
#endif
        ...
        if (opDesc & 060)
          {
            *mod_fault |= FR_ILL_MOD;
            //doFault (cpuPtr, FAULT_IPR, fst_ill_mod, "setupOperandDescriptor 30,31 MBZ");
          }
...
sim_debug (DBG_TRACEEXT, & cpu_dev, "No ARb %u\n", k - 1);
        ...
        address += getMFReg18 (cpuPtr, reg, false, true, mod_fault); // ID=1: disallow du, allow n,ic
        address &= AMASK;
        ...
        e -> op [k - 1] = EISRead (cpuPtr, & e -> addr [k - 1]);
    }
    else
    {
          e->addr [k - 1].mat = OperandRead;      // no ARs involved yet
sim_debug (DBG_TRACEEXT, & cpu_dev, "No ARa %u\n", k - 1);
    ...
    setupOperandDescriptorCache (cpuPtr, k);
}
    ...
    for (int i = 0; i < 3; i ++)
      {
        if (i < cpu.currentInstruction.info -> ndes)
          setupOperandDescriptor (cpuPtr, i + 1);
        else
          setupOperandDescriptorCache (cpuPtr, i + 1);
      }
#endif
  }

static void parseAlphanumericOperandDescriptor (cpu_state_t *cpuPtr, uint k, uint useTA, bool allowDU, fault_ipr_subtype_ *mod_fault)
  {
    EISstruct * e = & cpu.currentEISinstruction;
    word18 MFk = e -> MF [k - 1];
...
// XXX Handle N too big intelligently....
        e -> N [k - 1] = (uint) getMFReg36 (cpuPtr, reg, false, false, mod_fault); // RL=1: disallow du,n,ic
#ifdef EIS_PTR3
        switch (cpu.du.TAk[k-1])
#else
        switch (e -> TA [k - 1])
#endif
          {
            ...
            default:
#ifdef L68
              doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseAlphanumericOperandDescriptor TA 3");
#else
    ...
    //if (e->N [k - 1] == 0)
      //doFault (cpuPtr, FAULT_IPR, FR_ILL_PROC, "parseAlphanumericOperandDescriptor N 0");

    sim_debug (DBG_TRACEEXT, & cpu_dev, "N%u %o\n", k, e->N[k-1]);

    word36 r = getMFReg36 (cpuPtr, MFk & 017, allowDU, true, mod_fault); // allow du based on instruction, allow n,ic
...
// XXX Handle 'r' too big intelligently...
        ...
        case CTA6:
          if (CN >= 6)
#ifdef L68
            doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseAlphanumericOperandDescriptor TAn CTA6 CN >= 6");
#else
            *mod_fault |= FR_ILL_PROC;
#endif
        ...
        case CTA9:
          if (CN & 01)
#ifdef L68
            doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "parseAlphanumericOperandDescriptor CTA9 & CN odd");
#else
            *mod_fault |= FR_ILL_PROC;
#endif
          CN = (CN >> 1);
        ...
        default:
#ifdef L68
          doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseAlphanumericOperandDescriptor TA1 3");
#else
          *mod_fault |= FR_ILL_PROC;
#endif
...
static void parseArgOperandDescriptor (cpu_state_t *cpuPtr, uint k, fault_ipr_subtype_ *mod_fault)
  {
    PNL (L68_ (if (k == 1)
      DU_CYCLE_NLD1;
    else if (k == 2)
    ...
    word36 r = getMFReg36 (cpuPtr, yREG, false, true, mod_fault); // disallow du, allow n,ic
...
static void parseNumericOperandDescriptor (cpu_state_t *cpuPtr, int k, fault_ipr_subtype_ *mod_fault)
{
    PNL (L68_ (if (k == 1)
      DU_CYCLE_NLD1;
    else if (k == 2)
    ...
    {
        uint reg = opDesc & 017;
        e->N[k-1] = getMFReg18(cpuPtr, reg, false, false, mod_fault) & 077; // RL=1: disallow du,n,ic
    }
    else
        e->N[k-1] = opDesc & 077;
    ...
    word36 r = getMFReg36(cpuPtr, MFk & 017, false, true, mod_fault); // disallow du, allow n, ic
    if ((MFk & 017) == 4)   // reg == IC ?
    {
        address += r;
        address &= AMASK;
...
// handled in numeric instructions
    ...
    if (N == 0)
      {
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseNumericOperandDescriptor N=0");
      }
#endif
...
sim_printf ("k %d N %d S %d\n", k, N, S);
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseNumericOperandDescriptor N=1 S=0|1|2");
      }
#endif
...
// This breaks eis_tester 631 dtb; the S field in OP2 is ignored by the instruction.
    ...
    if (N == 2 && S == 0)
      {
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseNumericOperandDescriptor N=2 S=0");
      }
#endif
...
// handled in numeric instructions
    ...
    if (N == 3 && S == 0 && TN == 1)
      {
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseNumericOperandDescriptor N=3 S=0 TN 1");
      }
#endif
#endif
...
// XXX Handle 'r' too big intelligently...
        ...
        case CTN9:
            if (CN & 1u)
#ifdef L68
              doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "parseNumericOperandDescriptor CTA9 & CN odd");
#else
              *mod_fault |= FR_ILL_PROC;
#endif
            CN = (CN >> 1u) & 03u;
...
static void parseBitstringOperandDescriptor (cpu_state_t *cpuPtr, int k, fault_ipr_subtype_ *mod_fault)
{
    PNL (L68_ (if (k == 1)
      DU_CYCLE_ANLD1;
    else if (k == 2)
    ...
    {
        uint reg = opDesc & 017;
        e->N[k-1] = getMFReg36(cpuPtr, reg, false, false, mod_fault) & 077777777;  // RL=1: disallow du,n,ic
    ...
    if (B >= 9)
#ifdef L68
      doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "parseBitstringOperandDescriptor B >= 9");
#else
      *mod_fault |= FR_ILL_PROC;
#endif
...
static void cleanupOperandDescriptor (cpu_state_t *cpuPtr, int k)
  {
    EISstruct * e = & cpu.currentEISinstruction;
    if (e -> addr [k - 1].cacheValid && e -> addr [k - 1].cacheDirty)
      {
        EISWriteCache(cpuPtr, & e -> addr [k - 1]);
      }
...
//    cpu.AR [ARn].BITNO = tab [bitno];
    // SET_PR_BITNO (cpuPtr, cpuPtr, cpuPtr, ARn, bitFromCnt[bitno % 8]);
    SET_AR_CHAR_BITNO (cpuPtr, ARn, bitFromCnt[bitno % 8] / 9, bitFromCnt[bitno % 8] % 9);
    HDBGRegAR (ARn);
  }
...
void axbd (cpu_state_t *cpuPtr, uint sz)
  {
    ...
    word6 reg = GET_TD (cpu.cu.IWB); // 4-bit register modification (None except
                                  // au, qu, al, ql, xn)
    // r is the count of characters
    word36 rcnt = getCrAR (cpuPtr, reg);
    int32_t r;
...
//      }
    ...
    //SET_PR_BITNO (cpuPtr, cpuPtr, cpuPtr, ARn, sum % 36);
    SET_AR_CHAR_BITNO (cpuPtr, ARn, (word2)((sum % 36) / 9), (word2)(sum % 9));
    HDBGRegAR (ARn);
  }
        ...
        if (r % 36)
          {
            ...
            SET_AR_CHAR_BITNO (cpuPtr, ARn, (bits % 36) / 9,
                                    bits % 9);
          }
      }
    else
        ...
        if (r % 36)
          {
            ...
            SET_AR_CHAR_BITNO (cpuPtr, ARn, (r % 36) / 9,
                                    r % 9);
          }
      }
    HDBGRegAR (ARn);
...
sim_printf ("abd sum 0%o %d.\n", sum, sum);
    ...
    SET_AR_CHAR_BITNO (cpuPtr, ARn, (sum / 9) & MASK2, GET_AR_BITNO (ARn));
#else
    // Fails ISOLTS
    //SET_PR_BITNO (cpuPtr, cpuPtr, cpuPtr, ARn, sum % 36);
    SET_AR_CHAR_BITNO (cpuPtr, ARn, (sum % 36) / 9, sum % 9);
#endif
    HDBGRegAR (ARn);
...
// XXX This code is assuming that 'r' has 18 bits of data....
    int32_t r = (int32_t) (getCrAR (cpuPtr, reg) & MASK18);
    ...
    SET_AR_CHAR_BITNO (cpuPtr, ARn, 0, 0);
    HDBGRegAR (ARn);
  }
    ...
    if (GET_A (cpu.cu.IWB))
      {
        ...
        if (r % 36)
          {
            SET_AR_CHAR_BITNO (cpuPtr, ARn, (- ((bits % 36) / 9)) & MASK2,
                                    (- (bits % 9)) & MASK4);
          }
      }
    else
      {
        ...
        if (r % 36)
          {
            SET_AR_CHAR_BITNO (cpuPtr, ARn, (- ((r % 36) / 9)) & MASK2,
                                    (- (r % 9)) & MASK4);
          }
      }
    HDBGRegAR (ARn);
...
void s9bd (cpu_state_t *cpuPtr)
  {
    ...
    word21 r = getCrAR (cpuPtr, reg) & MASK21;;
    ...
    if (GET_A (cpu.cu.IWB))
      {
        ...
        //if (r % 36)
          //{
            ...
            SET_AR_CHAR_BITNO (cpuPtr, ARn, (GET_AR_CHAR (ARn) - r)  & MASK2, 0);
          //}
      }
    else
      {
        ...
        //if (r % 36)
          //{
            ...
            SET_AR_CHAR_BITNO (cpuPtr, ARn, (-r) & MASK2, 0);
          //}
      }
    //cpu.AR[ARn].BITNO = 0;
    HDBGRegAR (ARn);
        ...
        if (sz == 4)
          {
            ...
            SET_AR_CHAR_BITNO (cpuPtr, ARn, (word2) charno, (word4) bitno);
          }
        else
          {
...
void cmpc (cpu_state_t *cpuPtr)
  {
...
#ifndef EIS_SETUP
    setupOperandDescriptor (cpuPtr, 1, &mod_fault);
    setupOperandDescriptor (cpuPtr, 2, &mod_fault);
#endif
    parseAlphanumericOperandDescriptor (cpuPtr, 1, 1, false, &mod_fault);
    parseAlphanumericOperandDescriptor (cpuPtr, 2, 1, false, &mod_fault);
    ...
    if (IWB_IRODD & 0000600000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "cmpc 9-10 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "cmpc op1 23 MBZ");
...
// ISOLTS 880 test-04c sets bit 23.
    ...
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "cmpc op2 23 MBZ");
#endif
    ...
    for (; cpu.du.CHTALLY < min (e->N1, e->N2); cpu.du.CHTALLY ++)
      {
        word9 c1 = EISget469 (cpuPtr, 1, cpu.du.CHTALLY); // get Y-char1n
        word9 c2 = EISget469 (cpuPtr, 2, cpu.du.CHTALLY); // get Y-char2n
sim_debug (DBG_TRACEEXT, & cpu_dev, "cmpc tally %d c1 %03o c2 %03o\n", cpu.du.CHTALLY, c1, c2);
        if (c1 != c2)
          {
            CLR_I_ZERO;  // an inequality found
            SC_I_CARRY (c1 > c2);
            cleanupOperandDescriptor (cpuPtr, 1);
            cleanupOperandDescriptor (cpuPtr, 2);
            return;
          }
      }
...
void scd (cpu_state_t *cpuPtr)
  {
    ...
    setupOperandDescriptor (cpuPtr, 1, &mod_fault);
    setupOperandDescriptor (cpuPtr, 2, &mod_fault);
    setupOperandDescriptorCache (cpuPtr, 3);
#endif

    parseAlphanumericOperandDescriptor (cpuPtr, 1, 1, false, &mod_fault);
    parseAlphanumericOperandDescriptor (cpuPtr, 2, 1, true, &mod_fault); // use TA1
    parseArgOperandDescriptor (cpuPtr, 3, &mod_fault);
    ...
    if (IWB_IRODD & 0777600000000)
      {
        ...
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "scd 0-10 MBZ");
      }

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scd op1 23 MBZ");

    // Bits 18-28. 30-31 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scd op3 18-28. 30-31 MBZ");
    ...
    if (! (e -> MF2 & MFkID) && ((e -> MF2 & MFkREGMASK) == 3))  // MF2.du
      ...
      {
        c1 = EISget469 (cpuPtr, 2, 0);
        c2 = EISget469 (cpuPtr, 2, 1);
      }
    ...
    if (e -> N1)
      {
        ...
        for ( ; cpu.du.CHTALLY < limit; cpu.du.CHTALLY ++)
          {
            yCharn11 = EISget469 (cpuPtr, 1, cpu.du.CHTALLY);
            yCharn12 = EISget469 (cpuPtr, 1, cpu.du.CHTALLY + 1);
    ...
    EISWriteIdx (cpuPtr, & e -> ADDR3, 0, CY3, true);

    cleanupOperandDescriptor (cpuPtr, 1);
    cleanupOperandDescriptor (cpuPtr, 2);
    cleanupOperandDescriptor (cpuPtr, 3);
 }
    ...
    if (IWB_IRODD & 0777600000000)
      {
        ...
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "scdr 0-10 MBZ");
      }

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scdr op1 23 MBZ");

    // Bits 18-28. 30-31 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scdr op3 18-28. 30-31 MBZ");
    ...
    if (e -> N1)
      {
        ...
        for ( ; cpu.du.CHTALLY < limit; cpu.du.CHTALLY ++)
          {
            yCharn11 = EISget469 (cpuPtr, 1, limit - cpu.du.CHTALLY - 1);
            yCharn12 = EISget469 (cpuPtr, 1, limit - cpu.du.CHTALLY);
...
void scm (cpu_state_t *cpuPtr)
  {
    ...
    parseAlphanumericOperandDescriptor (cpuPtr, 1, 1, false, &mod_fault);
    parseAlphanumericOperandDescriptor (cpuPtr, 2, 1, true, &mod_fault);
    parseArgOperandDescriptor (cpuPtr, 3, &mod_fault);
    ...
    // Bits 9-10 MBZ
    if (IWB_IRODD & 0000600000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "scm 9-10 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scm op1 23 MBZ");

    // Bits 18-28, 39-31 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scm op3 18-28, 39-31 MBZ");
    ...
    for ( ; cpu.du.CHTALLY < limit; cpu.du.CHTALLY ++)
      {
        word9 yCharn1 = EISget469 (cpuPtr, 1, cpu.du.CHTALLY);
...
void scmr (cpu_state_t *cpuPtr)
  {
    ...
    // Bits 9-10 MBZ
    if (IWB_IRODD & 0000600000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "scmr 9-10 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scmr op1 23 MBZ");

    // Bits 18 of OP3 MBZ
    //if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000400000)
    //  doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scmr op3 18 MBZ");

    // Bits 18-28, 39-31 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "scmr op3 18-28, 39-31 MBZ");
    ...
    for ( ; cpu.du.CHTALLY < limit; cpu.du.CHTALLY ++)
      {
        word9 yCharn1 = EISget469 (cpuPtr, 1, limit - cpu.du.CHTALLY - 1);
        word9 c = ((~mask) & (yCharn1 ^ ctest)) & 0777;
        if (c == 0)
          {
            //00...0 → C(Y3)0,11
...
#if 0
static word9 xlate (cpu_state_t *cpuPtr, word36 * xlatTbl, uint dstTA, uint c)
  {
...
static word9 xlate (cpu_state_t *cpuPtr, EISaddr * xlatTbl, uint dstTA, uint c)
  {
    uint idx = (c / 4) & 0177;      // max 128-words (7-bit index)
    word36 entry = EISReadIdx(cpuPtr, xlatTbl, idx);
...
void tct (cpu_state_t *cpuPtr)
  {
...
#ifndef EIS_SETUP
    setupOperandDescriptor (cpuPtr, 1, &mod_fault);
    setupOperandDescriptorCache (cpuPtr, 2);
    setupOperandDescriptorCache (cpuPtr, 3);
#endif

    parseAlphanumericOperandDescriptor (cpuPtr, 1, 1, false, &mod_fault);
    parseArgOperandDescriptor (cpuPtr, 2, &mod_fault);
    parseArgOperandDescriptor (cpuPtr, 3, &mod_fault);
    ...
    if (IWB_IRODD & 0777777000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "tct 0-17 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "tct op1 23 MBZ");

    // Bits 18-28, 39-31 of OP2 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "tct op2 18-28, 39-31 MBZ");

    // Bits 18-28, 39-31 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "tct op3 18-28, 39-31 MBZ");
    ...
    for ( ; cpu.du.CHTALLY < e -> N1; cpu.du.CHTALLY ++)
      {
        word9 c = EISget469 (cpuPtr, 1, cpu.du.CHTALLY); // get src char
        ...
        word9 cout = xlate (cpuPtr, &e->ADDR2, CTA9, m);
...
void tctr (cpu_state_t *cpuPtr)
  {
    ...
    // Bits 0-17 MBZ
    if (IWB_IRODD & 0777777000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "tctr 0-17 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "tctr op1 23 MBZ");

    // Bits 18-28, 39-31 of OP2 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "tctr op2 18-28, 39-31 MBZ");

    // Bits 18-28, 39-31 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000777660)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "tctr op3 18-28, 39-31 MBZ");
    ...
    for ( ; cpu.du.CHTALLY < limit; cpu.du.CHTALLY ++)
      {
        word9 c = EISget469 (cpuPtr, 1, limit - cpu.du.CHTALLY - 1); // get src char
...
void mlr (cpu_state_t *cpuPtr)
  {
    ...
    parseAlphanumericOperandDescriptor(cpuPtr, 1, 1, false, &mod_fault);
    parseAlphanumericOperandDescriptor(cpuPtr, 2, 2, false, &mod_fault);
    ...
    if (IWB_IRODD & 0000200000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mlr 10 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mlr op1 23 MBZ");

    // Bit 23 of OP2 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mlr op2 23 MBZ");
    ...
    if ((cpu.du.CHTALLY % PGSZ) == 0 &&
...
#endif
      {
        ...
        while (cpu.du.CHTALLY < e -> N1)
          {
            word36 pg [PGSZ];
            EISReadPage (cpuPtr, & e -> ADDR1, cpu.du.CHTALLY / 4, pg);
            EISWritePage (cpuPtr, & e -> ADDR2, cpu.du.CHTALLY / 4, pg);
        ...
        for ( ; cpu.du.CHTALLY < e -> N2; cpu.du.CHTALLY += 4)
          {
            ...
            word36 w = EISReadIdx (cpuPtr, & e -> ADDR1, n);
            EISWriteIdx (cpuPtr, & e -> ADDR2, n, w, true);
          }
        cleanupOperandDescriptor (cpuPtr, 1);
        cleanupOperandDescriptor (cpuPtr, 2);
        // truncation fault check does need to be checked for here since
    ...
    for ( ; cpu.du.CHTALLY < min (e->N1, e->N2); cpu.du.CHTALLY ++)
      {
        ...
        if (e -> TA1 == e -> TA2)
#endif
          EISput469 (cpuPtr, 2, cpu.du.CHTALLY, c);
        else
          {
            ...
            EISput469 (cpuPtr, 2, cpu.du.CHTALLY, cout);
          }
      }
        ...
        for ( ; cpu.du.CHTALLY < e -> N2 ; cpu.du.CHTALLY ++)
          {
            ...
            if (ovp && (cpu.du.CHTALLY == e -> N2 - 1))
              {
                if (isNeg)
                  EISput469 (cpuPtr, 2, cpu.du.CHTALLY, 015); // 015 is decimal -
                else
                  EISput469 (cpuPtr, 2, cpu.du.CHTALLY, 014); // 014 is decimal +
              }
            else
              EISput469 (cpuPtr, 2, cpu.du.CHTALLY, fillT);
          }
    }
    cleanupOperandDescriptor (cpuPtr, 1);
    cleanupOperandDescriptor (cpuPtr, 2);
    ...
    if (e -> N1 > e -> N2)
      {
        SET_I_TRUNC;
        if (T && ! TST_I_OMASK)
          doFault (cpuPtr, FAULT_OFL, fst_zero, "mlr truncation fault");
      }
    else
      CLR_I_TRUNC;
  }
...
void mrl (cpu_state_t *cpuPtr)
  {
    ...
    // Bit 10 MBZ
    if (IWB_IRODD & 0000200000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mrl 10 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mrl op1 23 MBZ");

    // Bit 23 of OP2 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mrl op2 23 MBZ");
...
// instead of a byte at a time...
    ...
    for ( ; cpu.du.CHTALLY < min (e -> N1, e -> N2); cpu.du.CHTALLY ++)
      {
        word9 c = EISget469 (cpuPtr, 1, e -> N1 - cpu.du.CHTALLY - 1); // get src char
        word9 cout = 0;
        ...
        if (e -> TA1 == e -> TA2)
#endif
          EISput469 (cpuPtr, 2, e -> N2 - cpu.du.CHTALLY - 1, c);
        else
          {
            // If data types are dissimilar (TA1 ≠ TA2), each character is
            // high-order truncated or zero filled, as appropriate, as it is
...
// ISOLTS 838 01f, RJ78 p.11-154 - the rightmost digit is examined for overpunch.
            ...
            EISput469 (cpuPtr, 2, e -> N2 - cpu.du.CHTALLY - 1, cout);
          }
      }
        ...
        for ( ; cpu.du.CHTALLY < e -> N2 ; cpu.du.CHTALLY ++)
          {
            ...
            if (ovp && (cpu.du.CHTALLY == e -> N2 - 1))
              {
                if (isNeg)
                  EISput469 (cpuPtr, 2, e -> N2 - cpu.du.CHTALLY - 1, 015); // 015 is decimal -
                else
                  EISput469 (cpuPtr, 2, e -> N2 - cpu.du.CHTALLY - 1, 014); // 014 is decimal +
              }
            else
              {
                 EISput469 (cpuPtr, 2, e -> N2 - cpu.du.CHTALLY - 1, fillT);
              }
          }
    }
    cleanupOperandDescriptor (cpuPtr, 1);
    ...
    if (e -> N1 > e -> N2)
      {
        SET_I_TRUNC;
        if (T && ! TST_I_OMASK)
          doFault (cpuPtr, FAULT_OFL, fst_zero, "mrl truncation fault");
      }
    else
      CLR_I_TRUNC;
  }
...
static void EISloadInputBufferNumeric (cpu_state_t *cpuPtr, int k)
{
    EISstruct * e = & cpu.currentEISinstruction;
    ...
    for(int n = 0 ; n < N ; n += 1)
    {
        word9 c = EISget49(cpuPtr, a, &pos, TN);
        ...
        switch(S)
        {
            case CSFL:  // this is the real evil one ....
                ...
                else
                {
                    c &= 0xf;   // hack off all but lower 4 bits
                    if (c > 011)
                        doFault(cpuPtr, FAULT_IPR, fst_ill_dig, "loadInputBufferNumeric(2): illegal char in input"); // TODO: generate ill proc fault
            ...
            case CSLS:
                ...
                {
                    if (c < 012 || c > 017)
                        doFault(cpuPtr, FAULT_IPR, fst_ill_dig, "loadInputBufferNumeric(3): illegal char in input"); // TODO: generate ill proc fault
                ...
                {
                    if (c > 011)
                        doFault(cpuPtr, FAULT_IPR, fst_ill_dig, "loadInputBufferNumeric(4): illegal char in input");
                    *p++ = c; // store 4-bit char in buffer
                }
                break;
            ...
            case CSTS:
                ...
                {
                    if (c < 012 || c > 017)
                         doFault(cpuPtr, FAULT_IPR, fst_ill_dig, "loadInputBufferNumeric(5): illegal char in input");
                    if (c == 015)   // '-'
                        e->sign = -1;
                    e->srcTally -= 1;   // 1 less source char
                }
                ...
                {
                    if (c > 011)
                        doFault(cpuPtr, FAULT_IPR, fst_ill_dig, "loadInputBufferNumeric(6): illegal char in input");
                    *p++ = c; // store 4-bit char in buffer
                }
                break;
...
static void EISloadInputBufferAlphnumeric (cpu_state_t *cpuPtr, int k)
  {
    EISstruct * e = & cpu.currentEISinstruction;
    ...
    for (uint n = 0 ; n < N ; n ++)
      {
        word9 c = EISget469 (cpuPtr, k, n);
        * p ++ = c;
      }
}

static void EISwriteOutputBufferToMemory (cpu_state_t *cpuPtr, int k)
  {
    EISstruct * e = & cpu.currentEISinstruction;
    ...
    for (uint n = 0 ; n < (uint) e -> dstTally; n ++)
      {
        ...
        EISput469 (cpuPtr, k, n, c49);
      }
  }


static void writeToOutputBuffer (cpu_state_t *cpuPtr, word9 **dstAddr, int szSrc, int szDst, word9 c49)
{
    EISstruct * e = & cpu.currentEISinstruction;
...
static int mopCHT (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
...
#ifdef EIS_PTR2
        word9 entry = EISget49(cpuPtr, &e->ADDR2, &e->mopPos, CTN9);  // get mop table entries
#else
        word9 entry = EISget49(cpuPtr, e->mopAddress, &e->mopPos, CTN9);  // get mop table entries
#endif
...
static int mopENF (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    // For IF(0) = 0 (end floating-sign operation),
    if (!(e->mopIF & 010))
    {
        ...
        if (!e->mopES && !e->mopSN)
        {
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, e->editInsertionTable[2]);
            e->mopES = true;
        }
        // If ES is OFF and SN is ON, then edit insertion table entry 4 is moved to the receiving field and ES is set ON.
        if (!e->mopES && e->mopSN)
        {
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, e->editInsertionTable[3]);
            e->mopES = true;
        }
        // If ES is ON, no action is taken.
        ...
        if (!e->mopES)
        {
            ...
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, e->editInsertionTable[4]);
            e->mopES = true;
        }
        // If ES is ON, no action is taken.
    }
...
static int mopIGN (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
...
static int mopINSA (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    ...
    if (!e->mopES)
      {
        writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, e->editInsertionTable[1]);
...
#ifdef EIS_PTR2
            EISget49(cpuPtr, &e->ADDR2, &e->mopPos, CTN9);
#else
            EISget49(cpuPtr, e->mopAddress, &e->mopPos, CTN9);
#endif
            e->mopTally -= 1;
          }
      }
...
#ifdef EIS_PTR2
            word9 c = EISget49(cpuPtr, &e->ADDR2, &e->mopPos, CTN9);
#else
            word9 c = EISget49(cpuPtr, e->mopAddress, &e->mopPos, CTN9);
#endif
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, c);
            e->mopTally -= 1;
          }
    ...
    // entry (1-8) is to be moved to the receiving field.
          ...
          {
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, e->editInsertionTable[e->mopIF-1]);
          }
      }
#else
...
#else
#ifdef EIS_PTR2
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, EISget49(cpuPtr, &e->ADDR2, &e->mopPos, CTN9));
#else
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, EISget49(cpuPtr, e->mopAddress, &e->mopPos, CTN9));
#endif
#endif
            e->mopTally -= 1;
        }
...
static int mopINSB (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    // If C(IF) = 9-15, an IPR fault occurs.
    if (e->mopIF >= 9 && e->mopIF <= 15)
    ...
    if (!e->mopES)
    {
        ...
        // receiving field. If IF = 0, then the next 9 bits are also skipped.
        // If IF is not 0, the next 9 bits are treated as a MOP.
        writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, e->editInsertionTable[0]);
...
#else
            writeToOutputBuffer(cpuPtr, &e->out, 9, e->dstSZ, EISget49(cpuPtr, e->mopAddress, &e->mopPos, CTN9));
            //EcpuPtr, ISget49(e->mopAddress, &e->mopPos, CTN9);
#endif
            e->mopTally -= 1;
...
static int mopINSM (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF == 0)
        e->mopIF = 16;
...
static int mopINSN (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    // If C(IF) = 9-15, an IPR fault occurs.
    if (e->mopIF >= 9 && e->mopIF <= 15)
...
static int mopINSP (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    // If C(IF) = 9-15, an IPR fault occurs.
    if (e->mopIF >= 9 && e->mopIF <= 15)
...
static int mopLTE (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
...
#ifdef EIS_PTR2
    word9 next = EISget49(cpuPtr, &e->ADDR2, &e->mopPos, CTN9);
#else
    word9 next = EISget49(cpuPtr, e->mopAddress, &e->mopPos, CTN9);
#endif
    e->mopTally -= 1;
...
static int mopMFLC (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF == 0)
        e->mopIF = 16;
    ...
    for(int n = 0 ; n < e->mopIF ; n += 1)
    {
        ...
        if (!e->mopES) { // e->mopES is OFF
            ...
            if (isDecimalZero (c)) {
                ...
                writeToOutputBuffer(cpuPtr, &e->out, e->srcSZ, e->dstSZ, c);
...
static int mopMFLS (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF == 0)
        e->mopIF = 16;
...
static int mopMORS (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF == 0)
        e->mopIF = 16;
...
static int mopMVC (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF == 0)
        e->mopIF = 16;
...
//#if 0
        ...
        writeToOutputBuffer(cpuPtr, &e->out, e->srcSZ, e->dstSZ, *e->in);
        if (! isDecimalZero (*e->in))
            e->mopZ = false;
        e->in += 1;
...
static int mopMSES (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mvne == true)
        return mopMVC (cpuPtr);   // XXX I think!
...
static int mopMVZA (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF == 0)
        e->mopIF = 16;
...
static int mopMVZB (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF == 0)
        e->mopIF = 16;
...
static int mopSES (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    if (e->mopIF & 010)
        e->mopES = true;
...
static MOP_struct* EISgetMop (cpu_state_t *cpuPtr)
{
    EISstruct * e = & cpu.currentEISinstruction;
    ...
    {   // yep....
        ...
        p->data = EISRead(cpuPtr, &e->ADDR2);   // read it from memory
#else
        PNL (cpu.du.Dk_PTR_W[1] = (cpu.du.Dk_PTR_W[1] + 1) & AMASK);     // bump source to next address
        PNL (p->data = EISRead(cpuPtr, e->mopAddress));   // read it from memory
#ifdef EIS_PTR
        cpu.du.Dk_PTR_W[1] = (cpu.du.Dk_PTR_W[1] + 1) & AMASK;     // bump source to next address
        p->data = EISRead(cpuPtr, e->mopAddress);   // read it from memory
#else
...
#ifdef EIS_PTR2
static void mopExecutor (cpu_state_t *cpuPtr)
#else
static void mopExecutor (cpu_state_t *cpuPtr, int kMop)
#endif
  {
    EISstruct * e = & cpu.currentEISinstruction;
    ...
    while (e->dstTally)
    {
        ...
        int mres = m->f(cpuPtr);    // execute mop
...
//  mopTally 0 srcTally 32 dstTally 0 is not a fault
    ...
    if (e -> _faults)
      doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mopExecutor");
}
...
void mve (cpu_state_t *cpuPtr)
  {
    ...
    setupOperandDescriptor(cpuPtr, 1, &mod_fault);
    setupOperandDescriptor(cpuPtr, 2, &mod_fault);
    setupOperandDescriptor(cpuPtr, 3, &mod_fault);
#endif

    parseAlphanumericOperandDescriptor(cpuPtr, 1, 1, false, &mod_fault);
    parseAlphanumericOperandDescriptor(cpuPtr, 2, 2, false, &mod_fault);
    parseAlphanumericOperandDescriptor(cpuPtr, 3, 3, false, &mod_fault);
    ...
    if (IWB_IRODD & 0600600000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mve: 0, 1, 9, 10 MBZ");

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mve op1 23 MBZ");

#if 0
    // Bits 21-23 of OP1 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000070000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mve op2 21-23 MBZ");
#endif
    // only bit 23 according to RH03. this was fixed in DPS9000
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mve op2 23 MBZ");

    // Bit 23 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mve op3 23 MBZ");
    ...
    EISloadInputBufferAlphnumeric (cpuPtr, 1);   // according to MF1
    ...
    mopExecutor (cpuPtr);
#else
    mopExecutor (cpuPtr, 2);
#endif

    e -> dstTally = (int) e -> N3;  // restore dstTally for output

    EISwriteOutputBufferToMemory (cpuPtr, 3);
    cleanupOperandDescriptor (cpuPtr, 1);
    cleanupOperandDescriptor (cpuPtr, 2);
    cleanupOperandDescriptor (cpuPtr, 3);
  }
...
void mvne (cpu_state_t *cpuPtr)
  {
    ...
    parseNumericOperandDescriptor (cpuPtr, 1, &mod_fault);
    parseAlphanumericOperandDescriptor (cpuPtr, 2, 2, false, &mod_fault);
    parseAlphanumericOperandDescriptor (cpuPtr, 3, 3, false, &mod_fault);
    ...
    // Bits 0, 1, 9, and 10 MBZ
    if (IWB_IRODD & 0600600000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mvne: 0, 1, 9, 10 MBZ");


    // Bit 24-29 of OP1 MBZ
    // Multics has been observed to use 600162017511, cf RJ78
    //if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000007700)
      //doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvne op1 24-29 MBZ");

#if 0
    // Bits 21-29 of OP1 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000077700)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvne op2 21-29 MBZ");
#endif
    // only bits 21-23 according to RJ78, maybe even less on DPS8
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000070000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvne op2 21-23 MBZ");

#if 0
    // Bits 23-29 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000017700)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvne op3 23-29 MBZ");
#endif
    // only bit 23 according to RJ78
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvne op3 23 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mvne adjusted n1<1");
    ...
    if (e->N[1] == 0)
      doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mvne N2 0");

    // this is a flaw in DPS8/70 which was corrected in DPS88 and later
    // ISOLTS-841 07h, RH03 p.7-295
    if (e->N[2] == 0)
      doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mvne N3 0");
...
// 4 bit data in inBuffer, so signal the MOPS code of that.
    ...
    // 1. load sending string into inputBuffer
    EISloadInputBufferNumeric (cpuPtr, 1);   // according to MF1
    ...
    if (IWB_IRODD & 0000200000000)
      {
        ...
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mvt 10 MBZ");
      }
#else
    // Bits 0,1,9,10 MBZ
    if (IWB_IRODD & 0600600000000)
      {
        //sim_printf ("mvt %012"PRIo64"\n", IWB_IRODD);
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mvt 0,1,9,10 MBZ");
      }
#endif

    // Bit 23 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000010000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvt op1 23 MBZ");

// This breaks eis_tester mvt 110
#if 0
    // Bits 18 of OP2 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000400000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvt op2 18 MBZ");
#endif

    // Bits 18-28 of OP3 MBZ
    if (!(e->MF[2] & MFkID) && e -> op [2]  & 0000000777600)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "mvt op3 18-28 MBZ");
    ...
    if (lastpageidx>0)
        EISReadIdx(cpuPtr, &e->ADDR1, (uint)lastpageidx);
    // prepage xlate table
    EISReadIdx(cpuPtr, &e->ADDR3, 0);
    EISReadIdx(cpuPtr, &e->ADDR3, xlatSize-1);
    ...
    for ( ; cpu.du.CHTALLY < min(e->N1, e->N2); cpu.du.CHTALLY ++)
    {
        ...
        if (e->TA1 == e->TA2)
#endif
            EISput469(cpuPtr, 2, cpu.du.CHTALLY, xlate (cpuPtr, &e->ADDR3, dstTA, c));
        else
        {
            ...
            word9 cout = xlate(cpuPtr, &e->ADDR3, dstTA, (uint) cidx);
...
//            }
    ...
    if (e->N1 < e->N2)
    {
        word9 cfill = xlate(cpuPtr, &e->ADDR3, dstTA, fillT);
        switch (e->srcSZ)
        {
            case 6:
                switch(e->dstSZ)
        ...
        for( ; cpu.du.CHTALLY < e->N2 ; cpu.du.CHTALLY ++)
            EISput469 (cpuPtr, 2, cpu.du.CHTALLY, cfill);
    }
    ...
    if (e->N1 > e->N2)
      {
        SET_I_TRUNC;
        if (T && ! TST_I_OMASK)
          doFault(cpuPtr, FAULT_OFL, fst_zero, "mvt truncation fault");
      }
    else
      CLR_I_TRUNC;
  }
...
void cmpn (cpu_state_t *cpuPtr)
{
    ...
    parseNumericOperandDescriptor(cpuPtr, 1, &mod_fault);
    parseNumericOperandDescriptor(cpuPtr, 2, &mod_fault);
    ...
    // Bits 0-10 MBZ
    if (IWB_IRODD & 0777600000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "cmpn 0-10 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "cmpn adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "cmpn adjusted n2<1");
    ...
    decNumber *op1 = decBCD9ToNumber(cpuPtr, e->inBuffer, n1, sc1, &_1);
    if (e->sign == -1)
        op1->bits |= DECNEG;
    if (e->S1 == CSFL)
        op1->exponent = e->exponent;

    EISloadInputBufferNumeric (cpuPtr, 2);   // according to MF2

    decNumber *op2 = decBCD9ToNumber(cpuPtr, e->inBuffer, n2, sc2, &_2);
    if (e->sign == -1)
        op2->bits |= DECNEG;
    if (e->S2 == CSFL)
...
static void EISwrite4(cpu_state_t *cpuPtr, EISaddr *p, int *pos, word4 char4)
{
    word36 w;
...
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
    ...
    w = EISRead(cpuPtr, p);      // read dst memory into w
...
// AL39, Figure 2-3
    ...
    EISWriteIdx(cpuPtr, p, 0, w, true); // XXX this is the inefficient part!
...
static void EISwrite9(cpu_state_t *cpuPtr, EISaddr *p, int *pos, word9 char9)
{
    word36 w;
...
static void EISwrite49(cpu_state_t *cpuPtr, EISaddr *p, int *pos, int tn, word9 c49)
{
    switch(tn)
    {
        case CTN4:
            EISwrite4(cpuPtr, p, pos, (word4) c49);
            return;
        case CTN9:
            EISwrite9(cpuPtr, p, pos, c49);
            return;
    }
}
...
void mvn (cpu_state_t *cpuPtr)
{
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mvn adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mvn adjusted n2<1");
 ...
 // positive.
                    if (e->P)
                        // special +
                        EISwrite49 (cpuPtr, & e->ADDR2, & pos, (int) dstTN,
    ...
    for (int i = 0 ; i < n2 ; i ++)
        switch (dstTN)
        {
            case CTN4:
                ...
                break;
            case CTN9:
                EISwrite49 (cpuPtr, & e->ADDR2, & pos, (int) dstTN, (word9) res[i]);
                break;
        }
...
sim_debug (DBG_CAC, & cpu_dev, "Ovr %o\n", Ovr);
    ...
    if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
        doFault (cpuPtr, FAULT_OFL, fst_zero, "mvn truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault (cpuPtr, FAULT_OFL, fst_zero, "mvn over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault (cpuPtr, FAULT_OFL, fst_zero, "mvn overflow fault");
    }
}
...
// 1 1 1 1  Set
    ...
    parseBitstringOperandDescriptor (cpuPtr, 1, & mod_fault);
    parseBitstringOperandDescriptor (cpuPtr, 2, & mod_fault);
    ...
    if (IWB_IRODD & 0360200000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "csl 1-4,10 MBZ");
    ...
    for( ; cpu.du.CHTALLY < min(e->N1, e->N2); cpu.du.CHTALLY += 1)
      {
        bool b1 = EISgetBitRWN(cpuPtr, &e->ADDR1, true);
        e->ADDR2.mode = eRWreadBit;
        bool b2 = EISgetBitRWN(cpuPtr, &e->ADDR2, true);
        ...
        EISgetBitRWN(cpuPtr, &e->ADDR2, e->ADDR1.last_bit_posn == 35);    // write bit w/ addr increment to memory
    }
    ...
    if (e->N1 > e->N2)
      {
        ...
        SET_I_TRUNC;
        if (T && tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "csl truncation fault");
      }
    else
      {
        CLR_I_TRUNC;
...
static bool EISgetBitRWNR (cpu_state_t *cpuPtr, EISaddr * p, bool flush)
  {
    int baseCharPosn = (p -> cPos * 9);     // 9-bit char bit position
    int baseBitPosn = baseCharPosn + p -> bPos;
    baseBitPosn -= (int) cpu.du.CHTALLY;
...
void csr (cpu_state_t *cpuPtr)
  {
    ...
    // Bits 1-4 and 10 MBZ
    if (IWB_IRODD & 0360200000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "csr 1-4,10 MBZ");
    ...
    for( ; cpu.du.CHTALLY < min(e->N1, e->N2); cpu.du.CHTALLY += 1)
      {
        bool b1 = EISgetBitRWNR(cpuPtr, &e->ADDR1, true);

        e->ADDR2.mode = eRWreadBit;
        bool b2 = EISgetBitRWNR(cpuPtr, &e->ADDR2, true);
        ...
        // if ADDR1 is on a word boundary, it might fault on the next loop,
        // so we flush the write in case.
        EISgetBitRWNR(cpuPtr, &e->ADDR2, e->ADDR1.last_bit_posn == 0);
      }
    ...
    if (e->N1 > e->N2)
      {
        ...
        SET_I_TRUNC;
        if (T && tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "csr truncation fault");
      }
    else
      {
        CLR_I_TRUNC;
...
static bool EISgetBit(cpu_state_t *cpuPtr, EISaddr *p, int *cpos, int *bpos)
{
#ifdef EIS_PTR
    long eisaddr_idx = EISADDR_IDX (p);
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
...
void cmpb (cpu_state_t *cpuPtr)
{
    ...
    if (IWB_IRODD & 0377200000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "cmpb 1-8,10 MBZ");
...
// code behaves this way for the 'and' case.
    ...
    {
        bool b1 = EISgetBit (cpuPtr, &e->ADDR1, &charPosn1, &bitPosn1);
        bool b2 = EISgetBit (cpuPtr, &e->ADDR2, &charPosn2, &bitPosn2);
...
static void load9x(cpu_state_t *cpuPtr, int n, EISaddr *addr, int pos)
{
    EISstruct * e = & cpu.currentEISinstruction;
#ifdef NEED_128
...
if (eisaddr_idx < 0 || eisaddr_idx > 2) { sim_warn ("IDX1"); return }
    ...
    word36 data = EISRead(cpuPtr, addr);
...
#endif
            data = EISRead(cpuPtr, addr);    // read it from memory
        }
...
void btd (cpu_state_t *cpuPtr)
{
    ...
    if (IWB_IRODD & 0377600000000)
      {
        ...
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "btd 0-8 MBZ");
      }

    // Bits 21-29 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000077700)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "btd op1 21-29 MBZ");

    // Bits 24-29 of OP2 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000007700)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "btd op2 24-29 MBZ");

    if (e->S[1] == 0)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "btd op2 S=0");
    ...
    if (e->N1 == 0 || e->N1 > 8)
        doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "btd(1): N1 == 0 || N1 > 8");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "btd adjusted n2<1");
    ...
    load9x(cpuPtr, (int) e->N1, &e->ADDR1, (int) e->CN1);
            ...
            switch(dstTN)
            {
                case CTN4:
                    if (e->P) //If TN2 and S2 specify a 4-bit signed number and P = 1, then the 13(8) plus sign character is placed appropriately if the result of the operation is positive.
                        EISwrite49(cpuPtr, &e->ADDR2, &pos, (int) dstTN, (decNumberIsNegative(op1) && !decNumberIsZero(op1)) ? 015 : 013);  // special +
                    else
                        EISwrite49(cpuPtr, &e->ADDR2, &pos, (int) dstTN, (decNumberIsNegative(op1) && !decNumberIsZero(op1)) ? 015 : 014);  // default +
                    break;
                case CTN9:
                    EISwrite49(cpuPtr, &e->ADDR2, &pos, (int) dstTN, (decNumberIsNegative(op1) && !decNumberIsZero(op1)) ? '-' : '+');
                    break;
            }
            break;
    ...
    for(int i = 0 ; i < n2 ; i++)
        ...
        {
            case CTN4:
                EISwrite49(cpuPtr, &e->ADDR2, &pos, (int) dstTN, (word9) (res[i] - '0'));
                break;
            case CTN9:
                EISwrite49(cpuPtr, &e->ADDR2, &pos, (int) dstTN, (word9) res[i]);
                break;
    ...
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "btd overflow fault");
    }
}
    ...
    for(uint n = 0 ; n < e->N1 ; n += 1)
    {
        ...
        if (n == 0 && e->TN1 == CTN4 && e->S1 == CSLS)
        {
            ...
            switch (c)
            {
                ...
                default:
                    // not a leading sign
                    doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "loadDec(): no leading sign (1)");
            }
            pos += 1;           // onto next posotion
            continue;
        }
        ...
        if (n == 0 && e->TN1 == CTN9 && e->S1 == CSLS)
        {
            ...
            switch (c)
            {
                ...
                default:
                    // not a leading sign
                    doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "loadDec(): no leading sign (2)");
            }
            pos += 1;           // onto next posotion
            continue;
        }
        ...
        if (n == e->N1-1 && e->TN1 == CTN4 && e->S1 == CSTS)
        {
            ...
            switch (c)
            {
                ...
                default:
                    // not a trailing sign
                    doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "loadDec(): no trailing sign (1)");
            }
            break;
        }
        ...
        if (n == e->N1-1 && e->TN1 == CTN9 && e->S1 == CSTS)
        {
            ...
            switch (c)
            {
                ...
                default:
                    // not a trailing sign
                    doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "loadDec(): no trailing sign (2)");
            }
            break;
        }
...
void dtb (cpu_state_t *cpuPtr)
{
    ...
    if (mbz)
      {
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "dtb(): 0-10 MBZ");
      }

    // Bits 24-29 of OP1 MBZ
    if (!(e->MF[0] & MFkID) && e -> op [0]  & 0000000007700)
      {
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "dtb op1 24-29 MBZ");
      }

    // Bits 21-29 of OP2 MBZ
    if (!(e->MF[1] & MFkID) && e -> op [1]  & 0000000077700)
      {
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "dtb op2 21-29 MBZ");
       }
    ...
    if (e->S1 == 0 || e->SF1 != 0)
    {
        doFault(cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_PROC|mod_fault}, "dtb():  S1=0 or SF1!=0");
    }
    ...
    if (e->N2 == 0 || e->N2 > 8)
    {
        doFault(cpuPtr, FAULT_IPR, fst_ill_proc, "dtb():  N2 = 0 or N2 > 8 etc.");
    }
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "dtb adjusted n1<1");
...
#if 0
    decNumber _1;
    decNumber *op1 = decBCD9ToNumber(cpuPtr, e->inBuffer, n1, 0, &_1);
    if (e->sign == -1)
        op1->bits |= DECNEG;
    if (decNumberIsZero(op1))
        op1->exponent = 127;
...
sim_printf("dtb: N1 %d N2 %d nin %d CN1 %d CN2 %d msk %012"PRIo64" %012"PRIo64"\n",e->N1,e->N2,n1,e->CN1,e->CN2,(word36)((msk >> 36) & DMASK), (word36)(msk & DMASK));
...
#ifdef NEED_128
        EISwrite9(cpuPtr, &e->ADDR2, &pos, (word9) rshift_128 (x, (uint) shift).l & 0777);
#else
        EISwrite9(cpuPtr, &e->ADDR2, &pos, (word9) (x >> shift )& 0777);
#endif
        shift -= 9;
    }
    ...
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "dtb overflow fault");
    }
}
...
void ad2d (cpu_state_t *cpuPtr)
{
    ...
    // Bits 1-8 MBZ
    if (IWB_IRODD & 0377000000000)
      doFault (cpuPtr, FAULT_IPR, fst_ill_op, "ad2d 1-8 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "ad2d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "ad2d adjusted n2<1");
    ...
    switch(e->S2)
    {
        ...
        case CSLS:  // fixed-point, leading sign
            switch(dstTN)
            {
                case CTN4:
                    if (e->P) //If TN2 and S2 specify a 4-bit signed number and P = 1, then the 13(8) plus sign character is placed appropriately if the result of the operation is positive.
                        EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (decNumberIsNegative(op3) && !decNumberIsZero(op3)) ? 015 :  013);  // special +
                    else
                        EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (decNumberIsNegative(op3) && !decNumberIsZero(op3)) ? 015 :  014);  // default +
                    break;
                case CTN9:
                    EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (decNumberIsNegative(op3) && !decNumberIsZero(op3)) ? '-' : '+');
                    break;
            }
            break;
    ...
    for(int j = 0 ; j < n2 ; j++)
        ...
        {
            case CTN4:
                EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (word9) (res[j] - '0'));
                break;
            case CTN9:
                EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (word9) res[j]);
                break;
        }
    ...
    switch(e->S2)
    {
        ...
        case CSFL:  // floating-point, leading sign.
            ...
            {
                case CTN4:
                    EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (op3->exponent >> 4) & 0xf); // upper 4-bits
                    EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN,  op3->exponent       & 0xf); // lower 4-bits
                    break;
                case CTN9:
                    EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, op3->exponent & 0xff);    // write 8-bit exponent
                    break;
            }
            break;
    ...
    if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
      doFault(cpuPtr, FAULT_OFL, fst_zero, "ad2d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "ad2d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "ad2d overflow fault");
    }
}
...
void ad3d (cpu_state_t *cpuPtr)
{
    ...
    parseNumericOperandDescriptor(cpuPtr, 1, &mod_fault);
    parseNumericOperandDescriptor(cpuPtr, 2, &mod_fault);
    parseNumericOperandDescriptor(cpuPtr, 3, &mod_fault);
    ...
    if (IWB_IRODD & 0200000000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "ad3d(): 1 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "ad3d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "ad3d adjusted n2<1");
    ...
    if (n3 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "ad3d adjusted n3<1");
    ...
    for(int i = 0 ; i < n3 ; i++)
        ...
        {
        case CTN4:
            EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (word9) (res[i] - '0'));
            break;
        case CTN9:
            EISwrite49(cpuPtr, &e->ADDR3, &pos, (int) dstTN, (word9) res[i]);
            break;
        }
    ...
    if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
      doFault(cpuPtr, FAULT_OFL, fst_zero, "ad3d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "ad3d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "ad3d overflow fault");
    }
}
...
void sb2d (cpu_state_t *cpuPtr)
{
    ...
    if (IWB_IRODD & 0377000000000)
      {
        //sim_printf ("sb2d %012"PRIo64"\n", IWB_IRODD);
        doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "sb2d 0-8 MBZ");
      }
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "sb2d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "sb2d adjusted n2<1");
    ...
    if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
      doFault(cpuPtr, FAULT_OFL, fst_zero, "sb2d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "sb2d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "sb2d overflow fault");
    }
}
...
void sb3d (cpu_state_t *cpuPtr)
{
    ...
    // Bit 1 MBZ
    if (IWB_IRODD & 0200000000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "sb3d(): 1 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "sb3d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "sb3d adjusted n2<1");
    ...
    if (n3 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "sb3d adjusted n3<1");
    ...
    if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
      doFault(cpuPtr, FAULT_OFL, fst_zero, "sb3d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "sb3d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "sb3d overflow fault");
    }
}
...
void mp2d (cpu_state_t *cpuPtr)
{
    ...
    // Bits 1-8 MBZ
    if (IWB_IRODD & 0377000000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mp2d 1-8 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mp2d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mp2d adjusted n2<1");
    ...
    if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
      doFault(cpuPtr, FAULT_OFL, fst_zero, "mp2d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "mp2d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "mp2d overflow fault");
    }
}
...
void mp3d (cpu_state_t *cpuPtr)
{
    ...
    // Bit 1 MBZ
    if (IWB_IRODD & 0200000000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "mp3d(): 1 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mp3d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mp3d adjusted n2<1");
    ...
    if (n3 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "mp3d adjusted n3<1");
...
//    sim_printf("c3:%s\n", c3);
    ...
    if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
      doFault(cpuPtr, FAULT_OFL, fst_zero, "mp3d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "mp3d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "mp3d overflow fault");
    }
}
...
void dv2d (cpu_state_t *cpuPtr)
{
    ...
    if (IWB_IRODD & 0377400000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "dv2d 1-9 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "dv2d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "dv2d adjusted n2<1");
    ...
    decNumber *op1 = decBCD9ToNumber(cpuPtr, e->inBuffer, n1, sc1, &_1);    // divisor
    if (e->sign == -1)
        op1->bits |= DECNEG;
    if (e->S1 == CSFL)
        op1->exponent = e->exponent;
    ...
    if (decNumberIsZero(op1))
    {
        doFault(cpuPtr, FAULT_DIV, fst_zero, "dv2d division by 0");
    }
    ...
    decNumber *op2 = decBCD9ToNumber(cpuPtr, e->inBuffer, n2, sc2, &_2);    // dividend
    if (e->sign == -1)
        op2->bits |= DECNEG;
    if (e->S2 == CSFL)
        op2->exponent = e->exponent;
...
sim_debug (DBG_TRACEEXT, & cpu_dev, "dv2d S1 %d S2 %d N1 %d N2 %d clz1 %d clz2 %d E1 %d E2 %d SF2 %d NQ %d\n",e->S1,e->S2,e->N1,e->N2,clz1,clz2,op1->exponent,op2->exponent,e->SF2,NQ);
    ...
    if (NQ > 63)
        doFault(cpuPtr, FAULT_DIV, fst_zero, "dv2d NQ>63");
    ...
    //if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
    //  doFault(cpuPtr, FAULT_OFL, fst_zero, "dv2d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "dv2d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "dv2d overflow fault");
    }
}
...

{
    ...
    if (IWB_IRODD & 0200400000000)
      doFault (cpuPtr, FAULT_IPR, (_fault_subtype) {.fault_ipr_subtype=FR_ILL_OP|mod_fault}, "dv3d(): 1,9 MBZ");
    ...
    if (n1 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "dv3d adjusted n1<1");
    ...
    if (n2 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "dv3d adjusted n2<1");
    ...
    if (n3 < 1)
        doFault (cpuPtr, FAULT_IPR, fst_ill_proc, "dv3d adjusted n3<1");
    ...
    if (decNumberIsZero(op1))
    {
        doFault(cpuPtr, FAULT_DIV, fst_zero, "dv3d division by 0");
    }
...
sim_debug (DBG_TRACEEXT, & cpu_dev, "dv3d S1 %d S2 %d N1 %d N2 %d clz1 %d clz2 %d E1 %d E2 %d SF3 %d NQ %d\n",e->S1,e->S2,e->N1,e->N2,clz1,clz2,op1->exponent,op2->exponent,e->SF3,NQ);
    ...
    if (NQ > 63)
        doFault(cpuPtr, FAULT_DIV, fst_zero, "dv3d NQ>63");
    // Note: NQ is currently unused apart from this FAULT_DIV check. decNumber produces more digits than required, but they are then rounded/truncated
    ...
    //if (TST_I_TRUNC && T && tstOVFfault (cpuPtr))
    //  doFault(cpuPtr, FAULT_OFL, fst_zero, "dv3d truncation(overflow) fault");
    if (EOvr && tstOVFfault (cpuPtr))
        doFault(cpuPtr, FAULT_OFL, fst_zero, "dv3d over/underflow fault");
    if (Ovr)
    {
        SET_I_OFLOW;
        if (tstOVFfault (cpuPtr))
          doFault(cpuPtr, FAULT_OFL, fst_zero, "dv3d overflow fault");
    }
}
