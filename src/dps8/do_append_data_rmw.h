word24 do_append_data_rmw (word36 * data, uint nWords)
  {
    DCDstruct * i = & cpu.currentInstruction;
    DBGAPP ("do_append_cycle(Entry) thisCycle=APU_DATA_RMW\n");
...
HI:
    ...
    if (nWords == 1)
      {
        core_read_lock (finalAddress, data, "APU_DATA_RMW");
      }
    else
      {
        sim_warn("do_append_cycle: RMW nWords %d !=1\n", nWords);
        core_readN (finalAddress, data, nWords, "APU_DATA_RMW");
      }
#else
    core_readN (finalAddress, data, nWords, "APU_DATA_RMW");
#endif
