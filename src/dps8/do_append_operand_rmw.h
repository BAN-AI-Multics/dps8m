word24 do_append_operand_rmw (word36 * data, uint nWords)
  {
    DCDstruct * i = & cpu.currentInstruction;
    DBGAPP ("do_append_cycle(Entry) thisCycle=OPERAND_RWM\n");
...
HI:
    ...
    if (nWords == 1)
      {
        core_read_lock (finalAddress, data, "OPERAND_RMW");
      }
    else
      {
        sim_warn("do_append_cycle: RMW nWords %d !=1\n", nWords);
        core_readN (finalAddress, data, nWords, "OPERAND_RMW");
      }
#else
    core_readN (finalAddress, data, nWords, "OPERAND_RMW");
#endif
