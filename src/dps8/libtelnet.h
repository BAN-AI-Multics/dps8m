...
# else  /* if defined( __GNUC__ ) && !defined( USING_DPSPRINTF ) */
#  define TELNET_GNU_PRINTF(f, a)
#  define TELNET_GNU_SENTINEL
# endif /* if defined( __GNUC__ ) && !defined( USING_DPSPRINTF ) */
...
union telnet_event_t
{
   ...
   * Event type
   *
   * The type field determines which event structure fields have been filled
   * in.
   */
...
extern void telnet_send_text(telnet_t *telnet, const char *buffer,
                             size_t size);
