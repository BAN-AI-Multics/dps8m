...
static const size_t  _buffer_sizes_count
  = sizeof ( _buffer_sizes ) / sizeof ( _buffer_sizes[0] );

/* ######################################################################### */

/* error generation function */
static telnet_error_t
_error(telnet_t *telnet, unsigned line, const char *func, telnet_error_t err,
...
/* push bytes out */
static void
_send(telnet_t *telnet, const char *buffer, size_t size)
{
  telnet_event_t ev;
...
_check_telopt(telnet_t *telnet, unsigned char telopt, int us)
{
  int i;
...
static __inline__ telnet_rfc1143_t
_get_rfc1143(telnet_t *telnet, unsigned char telopt)
{
...
_set_rfc1143(telnet_t *telnet, unsigned char telopt, unsigned char us,
             unsigned char him)
{
      ...
      if (telnet->q[i].telopt == telopt)
        {
          ...
          telnet->flags &= (unsigned char)~( TELNET_FLAG_TRANSMIT_BINARY
...
#define QUANTUM  4
  ...
  if (i >= telnet->q_size)
    {
      /* Expand the size */
      if (( qtmp = (telnet_rfc1143_t *)realloc(
              telnet->q,
              sizeof ( telnet_rfc1143_t ) * ( telnet->q_size + QUANTUM )))
          == 0)
        {
          _error(
            telnet,
...
static __inline__ void
_send_negotiate(telnet_t *telnet, unsigned char cmd, unsigned char telopt)
{
...
static void
_negotiate(telnet_t *telnet, unsigned char telopt)
{
  telnet_event_t    ev;
  telnet_rfc1143_t  q;
 ...
 */
static int
_environ_telnet(telnet_t *telnet, unsigned char type, char *buffer,
                size_t size)
{
  telnet_event_t            ev;
  ...
  if ((unsigned)buffer[0] != TELNET_ENVIRON_SEND
      ...
      && (unsigned)buffer[0] != TELNET_ENVIRON_INFO)
    {
      _error(
        ...
        TELNET_EPROTOCOL,
        0,
        "telopt %ld subneg has invalid command",
        (long)type);
      return 0;
    }
  ...
  if ((unsigned)buffer[1] != TELNET_ENVIRON_VAR
      && (unsigned)buffer[1] != TELNET_ENVIRON_USERVAR)
    {
      _error(
        ...
        TELNET_EPROTOCOL,
        0,
        "telopt %d subneg missing variable type",
        type);
      return 0;
    }
  ...
  if (( values = (struct telnet_environ_t *)calloc(
          count,
...
static int
_ttype_telnet(telnet_t *telnet, const char *buffer, size_t size)
{
  telnet_event_t ev;
...
telnet_t *
telnet_init(const telnet_telopt_t *telopts, telnet_event_handler_t eh,
...
static telnet_error_t
_buffer_byte(telnet_t *telnet, unsigned char byte)
{
  char *new_buffer;
...
static void
_process(telnet_t *telnet, const char *buffer, size_t size)
{
  telnet_event_t  ev;
  unsigned char   byte;
  size_t          i, start;
...
void
telnet_recv(telnet_t *telnet, const char *buffer, size_t size)
{
...
/* send an iac command */
void
telnet_iac(telnet_t *telnet, unsigned char cmd)
{
...
/* send negotiation */
void
telnet_negotiate(telnet_t *telnet, unsigned char cmd, unsigned char telopt)
{
  telnet_rfc1143_t q;
...
void
telnet_send(telnet_t *telnet, const char *buffer, size_t size)
{
  size_t i, l;
...
void
telnet_send_text(telnet_t *telnet, const char *buffer, size_t size)
{
  size_t i, l;
...
void
telnet_begin_sb(telnet_t *telnet, unsigned char telopt)
{
  unsigned char sb[3];
...
int
telnet_vprintf(telnet_t *telnet, const char *fmt, va_list va)
{
  char   buffer[1024];
  char * output = buffer;
  int    rs, i, l;
...
/* see telnet_vprintf */
int
telnet_printf(telnet_t *telnet, const char *fmt, ...)
{
  va_list  va;
  int      rs;
...
int
telnet_raw_vprintf(telnet_t *telnet, const char *fmt, va_list va)
{
  char   buffer[1024];
  char * output = buffer;
  int    rs;
...
int
telnet_raw_printf(telnet_t *telnet, const char *fmt, ...)
{
  va_list  va;
  int      rs;
