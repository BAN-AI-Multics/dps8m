...
/* ------------------------------------------------------------------ */
decNumber *
decNumberFromInt32(decNumber *dn, Int in)
{
 uInt unsig;
...
decNumber *
decNumberFromUInt32(decNumber *dn, uInt uin)
{
...
/* ------------------------------------------------------------------ */
Int
decNumberToInt32(const decNumber *dn, decContext *set)
{
...
uInt
decNumberToUInt32(const decNumber *dn, decContext *set)
{
...
decNumberFromString(decNumber *dn, const char chars[], decContext *set)
{
 ...
 Unit         resbuff[SD2U(DECBUFFER + 9)]; // local buffer in case need temp
                                            // [+9 allows for ln() constants]
 Unit *       allocres  = NULL;             // -> alloc result, iff allocated
 Int          d         = 0;                // count of digits in decimal part
 const char * dotchar   = NULL;             // where dot was found
 const char * cfirst    = chars;            // -> first character of dec part
...
decNumber *
decNumberAdd(decNumber *res, const decNumber *lhs, const decNumber *rhs,
             decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberAnd(decNumber *res, const decNumber *lhs, const decNumber *rhs,
             decContext *set)
{
 ...
 if (lhs->exponent != 0 || decNumberIsSpecial(lhs)
     || decNumberIsNegative(lhs) || rhs->exponent != 0
     || decNumberIsSpecial(rhs) || decNumberIsNegative(rhs))
   {
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberCompare(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                 decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberCompareSignal(decNumber *res, const decNumber *lhs,
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberCompareTotal(decNumber *res, const decNumber *lhs,
                      const decNumber *rhs, decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberCompareTotalMag(decNumber *res, const decNumber *lhs,
                         const decNumber *rhs, decContext *set)
{
 uInt       status = 0;                // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberDivide(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberExp(decNumber *res, const decNumber *rhs, decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberFMA(decNumber *res, const decNumber *lhs, const decNumber *rhs,
             const decNumber *fhs, decContext *set)
{
 uInt       status = 0;        // accumulator
 ...
 do
   {  // protect allocated storage
    ...
    // Note sNaN has to go through addOp to shorten payload if necessary
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberInvert(decNumber *res, const decNumber *rhs, decContext *set)
{
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberLn(decNumber *res, const decNumber *rhs, decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberLogB(decNumber *res, const decNumber *rhs, decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberLog10(decNumber *res, const decNumber *rhs, decContext *set)
{
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberMax(decNumber *res, const decNumber *lhs, const decNumber *rhs,
             decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberMaxMag(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberMin(decNumber *res, const decNumber *lhs, const decNumber *rhs,
             decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberMinMag(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberNextMinus(decNumber *res, const decNumber *rhs, decContext *set)
{
 ...
 status         &= DEC_Invalid_operation | DEC_sNaN; // only sNaN Invalid plz
 if (status != 0)
   {
    decStatus(res, status, set);
   }
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberNextPlus(decNumber *res, const decNumber *rhs, decContext *set)
{
 decNumber  dtiny;           // constant
 decContext workset = *set;  // work
 uInt       status  = 0;     // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberNextToward(decNumber *res, const decNumber *lhs,
                    const decNumber *rhs, decContext *set)
{
 decNumber  dtiny;           // constant
 decContext workset = *set;  // work
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberOr(decNumber *res, const decNumber *lhs, const decNumber *rhs,
            decContext *set)
{
 const Unit * ua, *ub;     // -> operands
 const Unit * msua, *msub; // -> operand msus
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberMultiply(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                  decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberPower(decNumber *res, const decNumber *lhs, const decNumber *rhs,
               decContext *set)
{
#if DECSUBSET
   ...
   {  // protect allocated storage
         ...
         {                                      // rhs Infinity
          ...
          if (decNumberIsNegative(lhs)          // lhs<0
            ...
            { // lhs >=0
             ...
             decNumberZero(res);                      // prep for 0/1/Infinity
    ...
    if (!useint)
      {   // non-integral rhs
       ...
       // iteration so a few extra digits rarely causes an extra iteration)
       ...
       *dac->lsu = 1;          // ..
       ...
       for (i = 1;; i++)
         {             // for each bit [top bit ignored]
          ...
          decMultiplyOp(dac, dac, dac,
           &aset, &status);    // dac=dac*dac [square]
         }  /*i*/
           ...
           if (!set->extended && decNumberIsNegative(rhs))
             {
              if (status & DEC_Overflow)
                ...
                {  // trickier -- Underflow may or may not be set
                 status
                        &= ~( DEC_Underflow | DEC_Subnormal ); // [one or both]
                 status |= DEC_Overflow;
                }
             }
#endif /* if DECSUBSET */
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberQuantize(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                  decContext *set)
{
 uInt status = 0;  // accumulator
...
decNumber *
decNumberReduce(decNumber *res, const decNumber *rhs, decContext *set)
{
#if DECSUBSET
  decNumber *allocrhs = NULL; // non-NULL if rounded rhs allocated
#endif /* if DECSUBSET */
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberRescale(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                 decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberRemainder(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                   decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberRotate(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                decContext *set)
{
 uInt status = 0;  // accumulator
      ...
      {   // rhs is OK
       ...
       if (rotate != 0 && rotate != set->digits        // zero or full rotation
         ...
         {    // lhs was infinite
          ...
          Unit *msumax
           = res->lsu + D2U(set->digits) - 1;      //-V778  // rotation msu
          ...
          if (shift > 0)
            {                                // not an exact num of units
             uInt save = res->lsu[0]
                         % powers[shift];    //-V557 // save low digit(s)
             decShiftToLeast(res->lsu, D2U(res->digits), shift);
             if (shift > msudigits)
               {                             // msumax-1 needs >0 digits
                uInt rem
                  = save % powers[shift - msudigits];      // split save
                *msumax
                  = (Unit)( save
                           / powers[shift - msudigits] );  // and insert
                *( msumax - 1 )
                 = *( msumax - 1 )
                   + (Unit)( rem
                             * powers[DECDPUN
                ...
                *msumax
                 = *msumax
                   + (Unit)( save * powers[msudigits - shift] );   // [maybe
                                                                   // *1]
               }
            }     // digits shift needed
            ...
            {     // some to do
             ...
             if (shift > 0)
               {  // not an exact number of units
                uInt save
                        = res->lsu[0] % powers[shift];   // save low digit(s)
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberSameQuantum(decNumber *res, const decNumber *lhs,
                     const decNumber *rhs)
{
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberScaleB(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                decContext *set)
{
 ...
 // rhs must be an integer
 ...
 else
   {
    ...
    if (reqexp == BADINT                           // something bad ..
        ...
        || ( abs(reqexp) + 1 ) / 2
        > ( set->digits + set->emax ))             // .. or out of range
      {
       status = DEC_Invalid_operation;
      }
    else
       ...
       if (!decNumberIsInfinite(res))
         {                              // prepare to scale
          ...
          if ((( exp ^ reqexp ) >= 0 )  // same sign ...
              && (( exp ^ res->exponent ) < 0 ))
            {     // but result had different the calculation
                  // overflowed, so force right treatment ...
             if (exp < 0)
               {
                res->exponent = DEC_MIN_EMIN - DEC_MAX_DIGITS;
               }
             else
               {
                res->exponent = DEC_MAX_EMAX + 1;
               }
            }
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberShift(decNumber *res, const decNumber *lhs, const decNumber *rhs,
               decContext *set)
{
 uInt status = 0;  // accumulator
                ...
                if (res->digits + shift > set->digits)
                  {
                   ...
                   // that updated res->digits; may have gone to 1 (for
                   // a single digit or for zero
                  }

                if (res->digits > 1 || *res->lsu)  // if non-zero..
                  {
                   res->digits
                    = decShiftToMost(res->lsu, res->digits, shift);
                  }
               }  // partial left
            }     // left
          else
...
decNumberSquareRoot(decNumber *res, const decNumber *rhs, decContext *set)
{
 ...
 do
   {  // protect allocated storage
    ...
    // having the correct exponent so as to handle subnormals correctly
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberSubtract(decNumber *res, const decNumber *lhs, const decNumber *rhs,
                  decContext *set)
{
 uInt status = 0;  // accumulator
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberToIntegralExact(decNumber *res, const decNumber *rhs,
                         decContext *set)
{
 decNumber dn;
...
decNumber *
decNumberToIntegralValue(decNumber *res, const decNumber *rhs,
                         decContext *set)
{
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberXor(decNumber *res, const decNumber *lhs, const decNumber *rhs,
             decContext *set)
{
 const Unit *ua, *ub;      // -> operands
 const Unit *msua, *msub;  // -> operand msus
...
/* ------------------------------------------------------------------ */
enum decClass
decNumberClass(const decNumber *dn, decContext *set)
{
...
/* ------------------------------------------------------------------ */
const char *
decNumberClassToString(enum decClass eclass)
{
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberCopyAbs(decNumber *res, const decNumber *rhs)
{
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberCopyNegate(decNumber *res, const decNumber *rhs)
{
 decNumberCopy(res, rhs);
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberCopySign(decNumber *res, const decNumber *lhs, const decNumber *rhs)
{
 uByte sign;                      // rhs sign
...
/* ------------------------------------------------------------------ */
uByte *
decNumberGetBCD(const decNumber *dn, uByte *bcd)
{
...
/* ------------------------------------------------------------------ */
decNumber *
decNumberSetBCD(decNumber *dn, const uByte *bcd, uInt n)
{
...
/* ------------------------------------------------------------------ */
Int
decNumberIsNormal(const decNumber *dn, decContext *set)
{
...
/* ------------------------------------------------------------------ */
Int
decNumberIsSubnormal(const decNumber *dn, decContext *set)
{
 Int ae;      // adjusted exponent
...
/* ------------------------------------------------------------------ */
static decNumber *
decAddOp(decNumber *res, const decNumber *lhs, const decNumber *rhs,
         decContext *set, uByte negate, uInt *status)
{
#if DECSUBSET
  decNumber *alloclhs = NULL; // non-NULL if rounded lhs allocated
  decNumber *allocrhs = NULL; // .., rhs
 ...
 do
   {  // protect allocated storage
       ...
       if (!diffsign)
         {    // adding
          ...
          if (( partial <= DECDPUNMAX )   // result fits in unit
             ...
             *res->lsu
              = (Unit)partial;            // [copy could have overwritten RHS]
             break;
            }
    ...
    if (padding != 0)
      {
       ...
       if (padding < 0)
         {    // LHS needs the padding
          ...
          bits    = (uByte)( rhs->bits
                             ^ negate );  // assumed sign is now that of RHS
          t       = lhs;
          lhs     = rhs;
          rhs     = t;
         }
       ...
       exponent += \
         ( rhs->digits > reqdigits ) ? 0 : rhs->digits - reqdigits - 1;
       ...
       mult     = powers[padding
                         - ( rhsshift * DECDPUN )]; // this by multiplication
      }                                             // padding needed
    ...
    // Decide on the result buffer to use;
    // if possible place directly into result.
      ...
      { // subset arithmetic rounds from original significant digit
       ...
       if (res->digits < maxdigits)
       ...
       else
         {
          ...
          // maxdigits length, unless extended) before test for
          // rounding.
          if (res->digits > reqdigits)
            {
             res->digits = decGetDigits(acc, D2U(res->digits));
...
/* ------------------------------------------------------------------ */
static decNumber *
decDivideOp(decNumber *res, const decNumber *lhs, const decNumber *rhs,
         ...
         {    // RHS (divisor) is infinite
          ...
          if (op & ( REMAINDER | REMNEAR ))
            ...
            { // a division
             ...
             // for DIVIDEINT the exponent is always 0.  For DIVIDE,
             // result is a 0 with infinitely negative exponent, clamped
             // to minimum
             if (op & DIVIDE)
               {
      ...
      {                 // outer forever loop
       ...
       for (;;)
         {    // inner forever loop
            ...
            {  // unit-by-unit compare needed
             ...
             if (*pv1 == v2)
               { // var1 == var2
                 ...
                 // would increase digit by one, and the residue will be 0
                 // so the calculation is done; leave the loop with
                 // residue=0.
              ...
              // *pv1>v2.  Prepare for real subtraction; the lengths are
              // equal Estimate the multiplier (there's always a msu1-1)...
              // Bring in two units of var2 to provide a good estimate.
             mult = \
              (Int)((( eInt ) * msu1 * ( DECDPUNMAX + 1 ) + *( msu1 - 1 ))
                / msu2pair );
            } // lengths the same
          else
          ...
          decUnitAddSub(&var1[shift], var1units - shift, var2,
           var2units, 0, &var1[shift], -mult);
...
# if DECDPUN <= 4
                  if (( lsu - QUOT10(lsu, drop + 1) * powers[drop + 1] )
                      != 0)
                    {
...
# endif /* if DECDPUN <= 4 */
    ...
    else /* op!=DIVIDE */
      {
       ...
       if (op & ( REMAINDER | REMNEAR ))
         {
          // [Here, the exponent will be 0, because
          // var1 was adjusted appropriately.]
          Int postshift;       // work
          ...
          if (op & REMNEAR)
            {
             ...
             tarunits = decUnitAddSub(accnext, accunits, accnext,
              accunits, 0, accnext, 1);

             // Here, accnext (var1) holds tarunits Units with twice the
             // remainder's coefficient, which must now be compared to the
             // RHS.  The remainder's exponent may be smaller than the RHS's.
             compare = decUnitCompare(accnext, tarunits, rhs->lsu,
             ...
             if (compare > 0 || ( compare == 0 && wasodd ))
               {                              // adjustment needed
                ...
                accunits = -decUnitAddSub(accnext, accunits, rhs->lsu,
                 D2U(rhs->digits), expunits, accnext, -(Int)powers[exprem]);
                accdigits = decGetDigits(accnext, accunits);
                accunits = D2U(
                 accdigits);     // and recalculate the units for copy
                (void)accunits;
...
static decNumber *
decMultiplyOp(decNumber *res, const decNumber *lhs, const decNumber *rhs,
              decContext *set, uInt *status)
{
 ...
 Unit
  accbuff[SD2U(DECBUFFER * 4 + 1)]; // buffer (+1 for DECBUFFER==0,
...
# endif /* if DECDPUN == 1 */
        ...
        for (count = lhs->digits, cup = lhs->lsu, lip = zlhi; count > 0;
             lip++)
          {
        ...
        lmsi = lip - 1;       // save -> msi
        for (count = rhs->digits, cup = rhs->lsu, rip = zrhi; count > 0;
             rip++)
          {
...
/* ------------------------------------------------------------------ */
decNumber *
decExpOp(decNumber *res, const decNumber *rhs, decContext *set, uInt *status)
{
    ...
    *d->lsu     = 4;                 // set 4 ..
    ...
    if (h > 8)
      ...
      { // h<=8
       ...
       // As h is increased, the working precision below also increases
       // to compensate for the "constant digits at the front" effect.
...
/* ------------------------------------------------------------------ */
decNumber *
decLnOp(decNumber *res, const decNumber *rhs, decContext *set, uInt *status)
{
...
#define LN2  "0.6931471805599453094172321214581765680755"
    ...
    // Determine the working precision.  This is normally the requested
    // precision + 2, with a minimum of 9.  However, if the rhs is
    // 'over-precise' then allow for all its digits to potentially
    // participate (consider an rhs where all the excess digits are 9s)
    // so in this case use rhs->digits+2.
    p = MAXI(rhs->digits, MAXI(set->digits, 7)) + 2;

    // Allocate space for the accumulator and the high-precision
    // adjustment calculator, if necessary.  The accumulator must be
    // able to hold p digits, and the adjustment up to rhs->digits+p
    // digits.  They are also made big enough for 16 digits so that
    // they can be used for calculating the initial estimate.
    needbytes
    ...
    // now get top two digits of rhs into b by truncation, force into integer
...
/* ------------------------------------------------------------------ */
static decNumber *
decQuantizeOp(decNumber *res, const decNumber *lhs, const decNumber *rhs,
              decContext *set, Flag quant, uInt *status)
{
#if DECSUBSET
  decNumber *alloclhs   = NULL;         // non-NULL if rounded lhs allocated
  decNumber *allocrhs   = NULL;         // .., rhs
          ...
          if (res->exponent > reqexp)
            {
             ...
             res->digits
              = decShiftToMost(res->lsu, res->digits, 1);    // shift
      ...
      {                    // MAX or MIN, non-NaN result
       ...
       if (result == 0)
         { // operands are numerically equal
           // choose according to sign then exponent
...
/* ------------------------------------------------------------------ */
static Int
decCompare(const decNumber *lhs, const decNumber *rhs, Flag abs)
{
 ...
 compare = decUnitCompare(lhs->lsu, D2U(lhs->digits), rhs->lsu,
  D2U(rhs->digits), rhs->exponent - lhs->exponent);
 if (compare != BADINT)
   {
...
/* ------------------------------------------------------------------ */
static Int
decUnitCompare(const Unit *a, Int alength, const Unit *b, Int blength,
               Int exp)
{
 Unit *acc;                             // accumulator for result
 Unit accbuff[SD2U(DECBUFFER * 2 + 1)]; // local buffer
 Unit *allocacc = NULL;                 // -> alloc acc buffer, iff allocated
 ...
 // subtract [A+B*(-m)]
 accunits = \
  decUnitAddSub(a, alength, b, blength, expunits, acc, -(Int)powers[exprem]);
...
/* ------------------------------------------------------------------ */
static Int
decUnitAddSub(const Unit *a, Int alength, const Unit *b, Int blength,
              Int bshift, Unit *c, Int m)
{
 ...
 // For speed, do the addition as two loops; the first where both A and B
 // contribute, and the second (if necessary) where only one or other of the
 // numbers contribute. Carry handling is the same (duplicated) in each case.
 for (; c < minC; c++)
   {
    carry += *a;
    a++;
    ...
    for (; c < maxC; c++)
      {
       ...
       if ((ueInt)carry <= DECDPUNMAX)
        ...
        // negative case
        carry = carry
                + (eInt)( DECDPUNMAX + 1 ) * ( DECDPUNMAX + 1 );
        est   = (((ueInt)carry >> 11 ) * 53687 ) >> 18;
        *c    = (Unit)( carry - est * ( DECDPUNMAX + 1 ));
        carry = est - ( DECDPUNMAX + 1 ); // correctly negative
        if (*c < DECDPUNMAX + 1)
...
/* ------------------------------------------------------------------ */
static decNumber *
decTrim(decNumber *dn, decContext *set, Flag all, Flag noclamp, Int *dropped)
{
 Int d, exp;  // work
 uInt cut;    // ..
...
decShiftToMost(Unit *uar, Int digits, Int shift)
{
 ...
 if (cut == 0)
   ...
   {
    first
     = uar + D2U(digits + shift) - 1;  // where msu of source will end up
    for (; source >= uar; source--, target--)
...
static void
decSetCoeff(decNumber *dn, decContext *set, const Unit *lsu, Int len,
            Int *residue, uInt *status)
{
...
decApplyRound(decNumber *dn, decContext *set, Int residue, uInt *status)
{
 ...
 // all nines.  In this special case set to 100... explicitly and
 // adjust the exponent by one (as otherwise could overflow the array)
          ...
          if (dn->exponent + 1 == set->emin - set->digits + 1) //-V584
            {
             if (count == 1 && dn->digits == 1)
               ...
               {
                *sup = (Unit)powers[count - 1]
                       - 1;   // here 999.. in msu -> 99..
                dn->digits--;
               }

             dn->exponent++;
             *status |= DEC_Underflow | DEC_Subnormal | DEC_Inexact
                        | DEC_Rounded;
            }
...
decFinalize(decNumber *dn, decContext *set, Int *residue, uInt *status)
{
 ...
 // Must be careful, here, when checking the exponent as the adjusted
 // exponent could overflow 31 bits [because it may already be up to
 // twice the expected]. First test for subnormal.  This must be done
 // before any final round as the result could be rounded to Nmin or 0.
...
/* ------------------------------------------------------------------ */
static void
decSetMaxValue(decNumber *dn, decContext *set)
{
 Unit *up;                 // work
...
/* ------------------------------------------------------------------ */
static void
decSetSubnormal(decNumber *dn, decContext *set, Int *residue, uInt *status)
{
 decContext workset;  // work
 Int etiny, adjust;   // ..
...
/* ------------------------------------------------------------------ */
static uInt
decCheckMath(const decNumber *rhs, decContext *set, uInt *status)
{
 uInt save = *status;  // record

 if (set->digits > DEC_MAX_MATH || set->emax > DEC_MAX_MATH
...
#endif /* if DEC_MAX_EMAX > 999999999 */
#if DEC_MIN_EMIN < -999999999
# error GetInt may need updating [for Emin]
#endif /* if DEC_MIN_EMIN < -999999999 */
 if (ISZERO(dn))
   {
...
/* ------------------------------------------------------------------ */
static decNumber *
decDecap(decNumber *dn, Int drop)
{
...
/* ------------------------------------------------------------------ */
static Flag
decBiStr(const char *targ, const char *str1, const char *str2)
{
 for (;; targ++, str1++, str2++)
   {
...
/* ------------------------------------------------------------------ */
static decNumber *
decNaNs(decNumber *res, const decNumber *lhs, const decNumber *rhs,
        decContext *set, uInt *status)
{
