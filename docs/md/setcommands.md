...
#### ISOLTS_MODE
"**`ISOLTS_MODE`**" configures the CPU in such a way that facilitate running the *ISOLTS* factory diagnostics, where "`0`" or "`disable`" is disabled and "`1`" or "`enable`" is enabled:
...
when the hardware is properly functioning.  Unfortunately, without introducing a strict
